...
export function useSynth() {
  ...
  const whoosh = () => {
    ...
    const hp = mkBiquad("highpass", 400, 0.001);
    const bp = mkBiquad("bandpass", 1400, 6);
    const g = mkGain(0.08);
    ...
    g.gain.exponentialRampToValueAtTime(0.0006, t0 + 0.28);

    connect(noise, hp, bp, g, comp, ctx.destination);
    noise.start(t0);
    noise.stop(t0 + 0.3);
  };
  ...
  return { arm, select, whoosh, win, lose, draw };
}
