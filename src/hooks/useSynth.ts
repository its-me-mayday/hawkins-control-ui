...
export function useSynth() {
  ...
  const sfxVolRef = useRef<number>(0.9);
  ...
  const setEnabledSfx = (v: boolean) => {
    enabledRef.current = v;
  };

  const setSfxVolume = (v: number) => {
    sfxVolRef.current = Math.max(0, Math.min(1, v));
  };
  ...
  const mkGain = (v = 0.1) => {
    ...
    g.gain.value = v * sfxVolRef.current;
    return g;
  };

  const mkBiquad = (type: BiquadFilterType, freq: number, q = 0.0001) => {
    const f = ensureCtx().createBiquadFilter();
    f.type = type;
  ...
  const draw = () => {
    ...
    connect(o, f, g, ctx.destination);
    o.start(t0);
    o.stop(t0 + 0.18);
  };

  return { arm, select, win, lose, draw, setEnabledSfx, setSfxVolume };
}
