...
function ProgressCard({ label, progress = 0 }: { label: string; progress?: number }) {
  return (
    <div className="hk-card w-full max-w-[160px] sm:max-w-[200px] lg:max-w-[220px]">
      <div className="text-[11px] sm:text-xs uppercase tracking-widest text-(--hawkins-muted) mb-2">{label}</div>
      <div className="h-[140px] sm:h-[160px] lg:h-[180px] w-full grid place-items-center text-(--hawkins-muted)/80">Thinking…</div>
      <div className="mt-2">
        <div className="h-2 rounded bg-white/10 overflow-hidden">
          <div className="h-full rounded bg-(--hawkins-cyan) transition-[width] duration-100" style={{ width: `${Math.max(0, Math.min(100, progress ?? 0))}%` }} />
        </div>
      </div>
    </div>
  );
...
function DuelCard({
  ...
  if (!symbol) {
    return (
      <div className="flex justify-center">
        <div className="hk-card w-full max-w-[160px] sm:max-w-[200px] lg:max-w-[220px] grid place-items-center text-(--hawkins-muted)">
  ...
  const anim =
    locked && outcome === "PLAYER" && who === "PLAYER" ? "animate-card-win" :
    locked && outcome === "ENEMY" && who === "PLAYER" ? "animate-card-lose" :
    locked && outcome === "DRAW" ? "animate-card-draw" : "";
    ...
    <div className="flex justify-center">
      ...
      >
        <div className="relative w-full overflow-hidden rounded-lg grid place-items-center">
          <div className="h-[140px] sm:h-[160px] lg:h-[180px] w-full relative">
            <img
              ...
              alt={art.alt}
              loading="lazy"
              className={["h-full w-full", "object-contain", showLose ? "saturate-[.85]" : "", align === "right" ? "scale-x-[-1]" : ""].join(" ")}
        ...
        <div className="mt-2 text-center uppercase tracking-widest">
          <span className="text-[11px] sm:text-xs" style={{ color: accent }}>{symbol.replaceAll("_", " ")}</span>
        </div>
      </div>
    </div>
  );
...
export default function BattleDuel({ player, enemy, outcome = null, locked = false, thinking = false, progress = 0 }: Props) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] items-center gap-3 sm:gap-5">
      ...
      <div className="text-center opacity-80 text-(--hawkins-muted) uppercase tracking-widest text-[10px] sm:text-xs">VS</div>
      <div className="flex justify-center">
        {thinking && !enemy ? <ProgressCard label="Enemy thinking" progress={progress} /> : <DuelCard who="ENEMY" symbol={enemy ?? null} outcome={outcome} locked={locked} align="right" />}
      </div>
    </div>
  );
}
