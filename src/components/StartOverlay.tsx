type Props = { open: boolean; defaultTarget: number; onStart: (rounds: number) => void };
...
export default function StartOverlay({ open, defaultTarget, onStart }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center p-4" role="dialog" aria-modal="true">
      <div className="absolute inset-0 pointer-events-auto" style={{ background: "radial-gradient(70% 60% at 50% 40%, rgba(0,0,0,0.10), rgba(0,0,0,0.10) 38%, rgba(0,0,0,1) 68%)" }} />
      <div className="relative w-full max-w-md rounded-2xl p-6 border" style={{ background: "var(--hawkins-bg)", borderColor: "rgba(255,17,51,0.35)", boxShadow: "0 8px 26px rgba(0,0,0,.55), 0 0 22px rgba(255,17,51,.12), inset 0 0 0 rgba(0,0,0,0)" }}>
        <header className="text-center mb-4">
          <h2 className="hk-title text-xl">Start Match</h2>
          <p className="text-(--hawkins-muted) mt-1 text-xs uppercase tracking-widest">Choose how many rounds to win</p>
        </header>
